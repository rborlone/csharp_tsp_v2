    ...
    */
    ...
    public int Mutar(Random engine, Mapa m)
    {
        ...
        foreach (int cand in m.candidatosDeNodo[t1.idCiudad])
        {
            t2 = ady[cand];
            if (t1.siguiente == t2 || t0 == t2) continue;
            int g01 = m.data[t0.idCiudad][t1.idCiudad] - m.data[t1.idCiudad][t2.idCiudad];
            if (g01 > 0)
            {
                candidato.Add(new Tuple<int, Ciudad>(g01, t2));
            }
        }
    ...
    {
        largo = padre.largo;
        ady = InitializeArray<Ciudad>(largo);

        //estoy eligiendo 2 cortes distintos
        int corte1 = engine.Next(0, largo - 1);
        Ciudad ptr_p0 = padre.ady[corte1];
        int corte2 = corte1;
        while (corte2 == corte1)
            corte2 = engine.Next(0, largo - 1);
        ...
        List<int> reemplazos = new List<int>(largo);
        ...
        {
            Ciudad nodoHijo = ady[aux.idCiudad];
            ...
            if (aux.idCiudad != aux_m.idCiudad)
                reemplazos[aux.idCiudad] = aux_m.idCiudad;
            aux = aux.siguiente;
        ...
        {
            int ganancia = ThreeOpt(engine, m, mejor); //EPIC FAIL ESTABA TRABAJANDO CON EL 2OPT AL FINALIZAR EL PMX
            if (ganancia > 0)
            {
                nSinMejora = 0;
                costo -= ganancia;
